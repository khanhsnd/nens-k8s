...
`docs/ROADMAP.md` is the authoritative plan: phases 1–8 (shell, live resources, YAML, logs, exec + port-forward, discovery + CRDs, metrics + overview, Helm) are done, phase 9 (production hardening) is not. Read it before adding a feature — it already specifies the intended package layout and API shape for each phase.
