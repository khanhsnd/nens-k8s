...
`docs/ROADMAP.md` is the authoritative plan: phase 1 (shell) and phase 2 (live resources) are done, phases 3+ (YAML, logs, exec, CRDs, metrics, Helm) are not. Read it before adding a feature — it already specifies the intended package layout and API shape for each phase.
...
- `internal/kube/*` — adapters implementing those ports (`kubeconfig.Loader` → `KubeconfigSource` + `KubeconfigFiles`, `cluster.Registry` → `ClusterRegistry`, `resource.Store` → `ResourceSubscriber`). `internal/config` is the settings adapter (`SettingsStore`).
