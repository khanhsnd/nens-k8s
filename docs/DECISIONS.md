...
## Settings, appearance and what is remembered
...
it tracks its own label (see the grid-cell rule above — it is measured against the 30px row at the
...
`tab.store` saves `{sectionId, leafId}` pairs plus the active id through one `subscribe` rather than
a `save` in each action, and rebuilds the titles from `nav.model` on load — a leaf that no longer
exists is dropped, so a saved tab can never outlive its nav entry. `cluster.store` restores the
selection the same way but never calls `Connect`: connecting is the user's decision, and the tree
